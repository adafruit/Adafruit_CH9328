 ...
 */
void Adafruit_CH9328::begin(Stream *stream) { _stream = stream; }
...
void Adafruit_CH9328::sendKeyPress(byte keys[6], byte modifier) {
  byte dataPacket[8] = {modifier, 0x00,    keys[0], keys[1],
 ...
 *
 * @param command Pointer to the array of bytes representing the command to be
 * sent.
 ...
 * @return byte The corresponding keycode, or 0 if the character does not have a
 * direct keycode.
 */
byte Adafruit_CH9328::getKeyCode(char c) {
  ...
  if (isalpha(c)) {
    ...
    // Handle numeric characters; numbers are continuous in ASCII from '0' to
    // '9'
    ...
    case ' ':
      return KEY_SPACE;
    case '!':
      return KEY_1; // Shift is handled by getModifier
    case '@':
      return KEY_2; // Shift is handled by getModifier
    case '#':
      return KEY_3; // Shift is handled by getModifier
    case '$':
      return KEY_4; // Shift is handled by getModifier
    case '%':
      return KEY_5; // Shift is handled by getModifier
    case '^':
      return KEY_6; // Shift is handled by getModifier
    case '&':
      return KEY_7; // Shift is handled by getModifier
    case '*':
      return KEY_8; // Shift is handled by getModifier
    case '(':
      return KEY_9; // Shift is handled by getModifier
    case ')':
      return KEY_0; // Shift is handled by getModifier
    case '-':
      return KEY_MINUS;
    case '_':
      return KEY_MINUS; // Shift is handled by getModifier
    case '=':
      return KEY_EQUAL;
    case '+':
      return KEY_EQUAL; // Shift is handled by getModifier
    case '[':
      return KEY_LEFT_BRACE;
    case '{':
      return KEY_LEFT_BRACE; // Shift is handled by getModifier
    case ']':
      return KEY_RIGHT_BRACE;
    case '}':
      return KEY_RIGHT_BRACE; // Shift is handled by getModifier
    case '\\':
      return KEY_BACKSLASH;
    case '|':
      return KEY_BACKSLASH; // Shift is handled by getModifier
    case ';':
      return KEY_SEMICOLON;
    case ':':
      return KEY_SEMICOLON; // Shift is handled by getModifier
    case '\'':
      return KEY_QUOTE;
    case '"':
      return KEY_QUOTE; // Shift is handled by getModifier
    case ',':
      return KEY_COMMA;
    case '<':
      return KEY_COMMA; // Shift is handled by getModifier
    case '.':
      return KEY_PERIOD;
    case '>':
      return KEY_PERIOD; // Shift is handled by getModifier
    case '/':
      return KEY_SLASH;
    case '?':
      return KEY_SLASH; // Shift is handled by getModifier
    case '`':
      return KEY_TILDE;
    case '~':
      return KEY_TILDE; // Shift is handled by getModifier
    default:
      return 0; // No valid keycode (or handle as needed)
    }
  }
}
